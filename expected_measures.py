...
# Caso 4: Specifici bordi e spazi adiacenti
        ...
        if D[0] is None and not wall_right and j + 1 < 14 and all_directions_open(i, j + 1):
            if (j + 2 < 14 and i + 1 < 7 and cells[i + 1][j + 2]['left']) or (j + 2 < 14 and i - 1 >= 0 and cells[i - 1][j + 2]['left']):
                D[0] = 2.6

        if D[1] is None and not wall_top and i + 1 < 7 and all_directions_open(i + 1, j):
            if (i + 2 < 7 and j + 1 < 14 and cells[i + 2][j + 1]['bottom']) or (i + 2 < 7 and j - 1 >= 0 and cells[i + 2][j - 1]['bottom']):
                D[1] = 2.6

        if D[2] is None and not wall_bottom and i - 1 >= 0 and all_directions_open(i - 1, j):
            if (i - 2 >= 0 and j + 1 < 14 and cells[i - 2][j + 1]['top']) or (i - 2 >= 0 and j - 1 >= 0 and cells[i - 2][j - 1]['top']):
                D[2] = 2.6

        if D[3] is None and not wall_left and j - 1 >= 0 and all_directions_open(i, j - 1):
            if (j - 2 >= 0 and i + 1 < 7 and cells[i + 1][j - 2]['right']) or (j - 2 >= 0 and i - 1 >= 0 and cells[i - 1][j - 2]['right']):
                D[3] = 2.6
